...
async function createUserInterviewWithMeta({
    ...
    try {
        ...
        const userInterviewQuery = `
            ...
            RETURNING *
        `;
        const userInterviewResult = await client.query(userInterviewQuery, [interviewObjectId, userId]);
...
async function insertUserInterviewAnswer({ interviewQuestionId, answerObject }) {
    const client = await pgPool.connect();
    try {
        await client.query('BEGIN');
        ...
        const insertAnswerQuery = `
            ...
            RETURNING *
        `;
        const answerResult = await client.query(insertAnswerQuery, [interviewQuestionId, JSON.stringify(answerObject)]);
 ...
 */
async function insertUserInterviewResponseFeedback({ interviewResponseId, feedbackObject }) {
    const query = `
        INSERT INTO user_interview_responses_feedback (interview_response_id, feedback_object)
        VALUES ($1, $2)
        RETURNING *
    `;
    const { rows } = await pgQuery(query, [interviewResponseId, JSON.stringify(feedbackObject)]);
    return rows[0];
}
...
async function insertTabSwitch(userInterviewId, userId) {
    ...
    try {
        ...
        `;
        const { rows } = await client.query(countTabSwitchQuery, [userId, userInterviewId]);
